...
- **hl** — language code (`"fr"` or `"en"`). Threads through everything: model selection, INI filenames, PNG output names, translate direction. See [[invariants-and-traps]].
...
- **richTextArea PNG** — `richTextArea_<hl><N>.png`. The branded social image, captured page-by-page from the card surface by html2canvas and written by `webapi.save_capture`. Page 1 is what image-mode publishing attaches. See [[image-card-capture]].
...
- **Post (base) + adapters** — `post.py`'s `Post` is the base for `PostBsky`, `PostX`, and `PostFB` (all inherit). `PostFB` is the Graph API Facebook Page adapter (text/image, no threads). See [[post-base]], [[facebook-adapter]].
